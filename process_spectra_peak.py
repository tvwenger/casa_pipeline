...
def main(field,region,spws=[],stackedspws=[],stackedlabels=[],
   ...
   lines. Compute electron temperature.
    ...
    #
    # Get average rest frequency of stacked lines
    #
    stackedrestfreqs = []
    for my_stackedspws in stackedspws:
        restfreqs = [float(allrestfreqs[alllinespws.index(spw)].replace('MHz','')) for spw in my_stackedspws.split(',')]
        stackedrestfreqs.append(np.mean(restfreqs))
    #
    # Set-up file
    #
    with open(outfile,'w') as f:
        ...
        # Fit stacked RRLs
        #
        for my_stackedspws, stackedlabel, stackedrestfreq in zip(stackedspws,stackedlabels,stackedrestfreqs):
